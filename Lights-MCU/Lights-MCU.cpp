...
#define MULTI_CORE_FLAG_VALUE 123

void core1_entry(void){
    mutex_enter_blocking(&uart_mutex);
    printf("Mutex Value is init'd %b\n", mutex_is_initialized(&uart_mutex));
    ...
    while (true){
        ...
        if (uart_is_readable(uart0)){
            ...
            if (recv_char != sentinel){
                result.push_back(recv_char);
            }else{
                mutex_enter_blocking(&uart_mutex);
                uart_puts(uart0, result.c_str());
                uart_putc(uart0, '\n');
                mutex_exit(&uart_mutex);
                result.clear();
            }
            ...
            mutex_enter_blocking(&uart_mutex);
            uart_puts(uart0, "\tCore 1 is looping!\n");
            mutex_exit(&uart_mutex);
            sleep_ms(250);
        }
...
int main()
{
    ...
    // I am already doing 1, 2, dont do 3, or 4
    while (true) {
        mutex_enter_blocking(&uart_mutex);
